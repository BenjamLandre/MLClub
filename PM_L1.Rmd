...
For $\alpha$=1, a LASSO model is defined and a L1 regularization is applied.  
<br>
The L1 regularization shrink the coefficient uniformly as $\lambda$ increases.    
LASSO models are particularly interesting when among a great number of predictors, only a few have non-zero coefficients.   
...
<h4> Code </h4>   
LASSO methods is divided on 3 steps:    
<br>
...
###
<h2>Cross-validated LASSO model</h2>
...
Training and validation data sets must be created   
  
**(2)**    <span style="color:#d62d20">Training step</span>    
...
**(3)**    <span style="color:#d62d20">Validation step</span>    
...
-   alpha: the mixing parameter (alpha = 1 = LASSO penalty).    
...
# Number of variables with non-null coefficient are indicated on the top of the graph
plot(CV.LASSO)

# Coefficients for the LASSO model at minimal lambda
predict(CV.LASSO, type = "coef", s = "lambda.min")
```
```{r, echo = F}
...
plot(y = GLM$coefficients[-1], x = (1:11)+0.2, pch = "", axes = FALSE, xlim = c(0, 12), 
     xlab = "Variable", ylab = "Coefficient", main = "Comparison of GLM (black) and LASSO (red) coefficients.")
...
axis(1, (1:11)+0.1, col = NA, 
     ...
     col.ticks = 1)
segments(x0 = 1:11, x1 = 1:11, y1 = Coef_LASSO[-c(1,3, 5, 8, 12, 14)], y0 = 0, lty = "dotted",)
...
points(y = Coef_LASSO[-c(1,3, 5, 8, 12, 14)], x = 1:11, col = "red", pch = 4, cex = 1.5, lwd = 2)
```
...
**(3) Predicting on validation data set**   
...
```{r, echo = F}
# Prediction and summary statistics in training dataset
# heart_train$SC_log_pred <- predict(CV.LASSO, newx = train_dm, s = "lambda.min")
...
```{r}
# Prediction and summary statistics in testing dataset
heart_test$SC_log_pred <- predict(CV.LASSO, newx = test_dm, s = "lambda.min")
...
# Cross-validation of LASSO model
CV.LASSO <- cv.glmnet(x = x_train_dm,
                      y = y_train,
                      family = "cox",
                      nfolds = 10,
                      standardize = F,
...
```{r}
# Prediction and summary statistics in training dataset
Pred_LASSO <- predict(CV.LASSO, newx = x_train_dm, s = "lambda.min")
apply(Pred_LASSO, 2, Cindex, y= y_train)
```
...
```{r}
# Prediction and summary statistics in testing dataset
Pred_LASSO <- predict(CV.LASSO, newx = x_test_dm, s = "lambda.min")
apply(Pred_LASSO, 2, Cindex, y= y_test)
```
